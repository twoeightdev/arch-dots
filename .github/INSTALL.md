...
```
  ...
  ```javascript
  - $ efibootmgr -d /dev/nvme0n1 -p Y -c -L "Arch" -l /vmlinuz-linux -u 'root=UUID=XXXXXX-XXX rw quiet loglevel=3 nvidia-drm.modeset=1 rd.systemd.show_status=false rd.udev.log_priority=3 initrd=\intel-ucode.img initrd=\initramfs-linux.img' --verbose
  ```
